...
def clone_branch(repo, branch):
    sub_run("git clone -b {} https://github.com/42-AI/{}.git".format(branch, repo))
...
def main():
    ...
    for _dir in repos:
        ...
        for k, v in days.items():
            ...
            res = sub_run("python3 pdf_builder -b \"{}\" -d {} -t \"Day00 - ddd\" -o {}".format(_dir.replace('_', ' ').replace('-', ' '), v, dd))
            ...
            if pattern.findall(res.stderr.decode().strip()):
                ...
                # rr = pattern.findall(res.stderr.decode().strip())
                # print(rr)
                # if len(rr) == 0:
                #     #pattern = re.compile(r'(TeX)', re.DOTALL)
                #     rr = res.stdout.decode().strip()
                # print("\n".join(rr)[0].split('\n')[1:])
    #sub_run("rm -rf bootcamp*/")
    res = sub_run("python3 pdf_builder -b \"Data Engineering\" -f bootcamp_data-engineering/day00/ex03/psycopg2_basics.md -t \"Day00 - ddd\" -o bootcamp_data-engineering.psycopg2_doc.pdf")
    pattern = re.compile(r'(Successfully.*?!)')
    if pattern.findall(res.stderr.decode().strip()):
        print("{}SUCCESS{} !".format(OKGREEN, ENDC))
    else:
